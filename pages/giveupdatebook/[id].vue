...
const readBooks = async () => {
	try {
		...
		const response = await axios.get(`http://localhost:8181/givebookread/${id}`)
		book.value = response.data.rows
		name.value = response.data.rows[0].name
		namestudent.value = response.data.rows[0].namestudent
		speciality.value = response.data.rows[0].speciality
		course.value = response.data.rows[0].course
		namebook.value = response.data.rows[0].namebook
		...
		returnbook.value = response.data.rows[0].returnbook
		phone.value = response.data.rows[0].phone
	} catch (err) {
		console.log(err)
	}
}
const UpdateBook = async () => {
	try {
		...
		const response = await axios.put(
			`http://localhost:8181/updategivebook/${id}`,
			{
