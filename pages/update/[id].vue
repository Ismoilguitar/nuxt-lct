<template>
	<nav
		class="none hidden z-50 w-full bg-white border-b border-gray-200 dark:bg-gray-800 dark:border-gray-700"
	>
			...
			<div class="flex items-center justify-between">
				<div class="flex items-center justify-start rtl:justify-end">
					...
					<a class="flex ms-2 md:me-24">
						<img src="./public/лого.png" class="h-8 me-3" alt="ЛОГО" />
						<span
	...
	<div class="flex">
		...
		<Sidebar />
		<div class="ml-12">
			<h1 class="text-xl font-semibold ml-8">Cаҳифа барои таҳрири китоб</h1>
			<div
						...
						<div class="bg-white shadow-lg p-4 px-4 rounded-3xl md:p-8 mb-6">
							...
							>
								<div class="text-gray-600">
									<div
										v-if="photo"
										class="flex-shrink-0 relative w-full rounded-t-xl -mt-28 overflow-hidden pt-[40%] sm:rounded-s-xl sm:max-w-60 md:rounded-se-none md:max-w-xs"
									>
									...
									>
										...
										<div class="md:col-span-5 text-right">
											<div class="inline-flex items-end">
												<Toaster />
												<Button type="submit" variant="successgreen" size="lg"
...
const readBooks = async () => {
	try {
		...
		const response = await axios.get(`http://localhost:8181/read/${id}`)
		book.value = response.data.rows
		console.log('res', response.data.rows[0].datetime)
...
const UpdateBook = async () => {
	try {
		...
		toast({
			title: 'Китоб бомуваффақият таҳрир шуд!',
